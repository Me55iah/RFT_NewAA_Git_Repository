...
import resources.Motor.Your_Quote_Details_Confirm_viewHelper;
 ...
 * @author horned
 */
public class Your_Quote_Details_Confirm_view extends Your_Quote_Details_Confirm_viewHelper
{
	/**
	 * Script Name   : <b>Your_Quote_Details_Confirm_view</b>
	 * Generated     : <b>21 Jan 2015 13:30:09</b>
	 * Description   : Functional Test Script
	...
	{
		//------------------------------------------------------------------------------
		// ADDITIONS ADDED BY DAVE TO CONFIRM TOTALS MATCH ON QUOTE PAGE    -    Jan 2015 - FOR VIEW (Mod 21/1/15)
		//------------------------------------------------------------------------------
		
		// # # # # # # #    WILL MODULARISE FOR RE-USE WHEN TIME PERMITS :-) # # # # # # #
		
		//------------------------------------------------------------------------------------------
		BrowserSync(); // html_aside
		//------------------------------------------------------------------------------------------
		sleep(3);
		
		// // Debug
		Common_Variables.paymentType = "Single Payment"; // //  //AA Instalment Account (TIA)"; // Single Payment"; // 
		// // Debug
...
//		
//		String sVoluntaryExcessCode_Value = (String)list_aaMotorVoluntaryExcessCod().getProperty(".value");
//				
//		String[] sCoEx = sExcessValues.split("compulsory excess");
//		String sCE = sCoEx[1]; String sCompulsoryExcessValues = sCE.substring(1, 4); 
//		
//		String[] sToEx = sExcessValues.split("total excess");
//		String sTE = sToEx[1]; String sTotalExcessValues = sTE.substring(1, 4); 
//		
...
//		Common_Variables.policyTotalExcess = sTotalExcessValues;
		...
		//---------------------------------------------------------------------------
		// Detect SPLIT on Quote Retrievals = TRUE from DP
		
		if ((Common_Variables.retrieveQuote) == (true)){
			System.out.println(" - Page Split Detected - Changing to Known View -");
			sleep(2);
		...
		//if (label_totalExcess().isShowing()){  //||(list_lH_ExtraCover_VoluntaryEx
		//}
		
		// list_lH_ExtraCover_VoluntaryEx()
		...
		String sLH_ExtraCover_MotorLegalAssistance = (String)html_lcheckcontainerMLP().getProperty(".text"); //label_lH_ExtraCover_MLA		 
		...
		String sLH_ExtraCover_PersonalAccidentCover = (String)html_lcheckcontainerMAP().getProperty(".text");	// label_lH_ExtraCover_PAC	 
		...
		String sLH_ExtraCover_CarHireCover = (String)html_lcheckcontainerHIR().getProperty(".text");	//label_lH_ExtraCover_CarHire		 
		// .text:  Car Hire Cover £2.82 a month £29.99 Cover to keep you on the road if your car is written off or stolen.
		String sLH_ExtraCover_ProtectYourPolicyExcess = (String)html_lcheckcontainerXP().getProperty(".text"); // label_lH_ExtraCover_PYPExcess
		...
		String sLH_ExtraCover_MotorProtectNCD = (String)list_lH_ExtraCover_MotorP().getProperty(".value");		 
		// .value:  Not Protected
		String sLH_ExtraCover_MotorCoverType = (String)list_lH_ExtraCover_MotorCoverT().getProperty(".value");		 
		// .value:  Comprehensive
		String sLH_ExtraCover_MotorVoluntaryExcess = (String)list_aaMotorVoluntaryExcessCod().getProperty(".value");
		//------------------------------------------------------------------------------
...
//		}
		String sRH_AddonListISummary = null;
		...
		System.out.println(" --------------------------------------------------");// 
//		int iChkBoxCount = 0;
//		if (sMotorLegalAssistance_ClassName.equalsIgnoreCase("lcheck-label lcheckselected")){ // lcheck-label topround-corners5 lcheckselected
//			iChkBoxCount=iChkBoxCount+1; System.out.println(" Extra Cover Motor Legal Assistance CheckBox Was Selected");
//		}
//		if (sPersonalAccidentCover_ClassName.equalsIgnoreCase("lcheck-label lcheckselected")){ // lcheck-label topround-corners5 lcheckselected
//			iChkBoxCount=iChkBoxCount+1; System.out.println(" Extra Cover Personal Accident Cover CheckBox Was Selected");
//
//		}
//		if (sCarHireCover_ClassName.equalsIgnoreCase("lcheck-label lcheckselected")){ // lcheck-label topround-corners5 lcheckselected
//			iChkBoxCount=iChkBoxCount+1; System.out.println(" Extra Cover Care Hire Cover CheckBox Was Selected");
//
//		}
//		if (sProtectYourPolicyExcess_ClassName.equalsIgnoreCase("lcheck-label lcheckselected")){ // lcheck-label topround-corners5 lcheckselected
//			iChkBoxCount=iChkBoxCount+1; System.out.println(" Extra Cover Protect Your Policy CheckBox Was Selected");
//
//		}
		i = 1; // Always has a total payment amount
		i = i-1;  // Increment from above, starting index = 1 + 1st Item In List Is The Quote - Addons
		System.out.println(i+" Extra Cover CheckBox(s) Selected");
		...
		if ((Common_Variables.retrieveQuote) == (true)){
			System.out.println(" --------------------------------------------------");// 
			System.out.println(" * * * This Quote ("+sQuoteReferenceID+") Was Retrieved For Verification Successfully * * * ");// 
		}		
		System.out.println(" ==========================================================");// 
		//---------------------------------------------------------------------------------------------------------------------
		// RH Details Pane
		callScript("Motor.Extra_Verification");
		// Vehicle Details
